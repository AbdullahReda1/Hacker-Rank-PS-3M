...
int* matchingStrings(int strings_count, char** strings, int queries_count, char** queries, int* result_count) {
    ...
    *result_count = queries_count;
    ...
    for (int q_it = 0; q_it < queries_count; q_it++) {
        strcounter = 0;
        int query_length = strlen(queries[q_it]);

        for (int s_it = 0; s_it < strings_count; s_it++) {
            if (strlen(strings[s_it]) == query_length) {
                charcounter = 0;
                for (int k = 0; k < strlen(queries[q_it]); k++) {
                    if (strings[s_it][k] == queries[q_it][k]) {
                        charcounter++;
                    }
                }
                if (charcounter == strlen(queries[q_it])) {
                    strcounter++;
                }
            }
        }
        
        result[q_it] = strcounter;
    }
...
/*
    ...
    *result_count = queries_count;
    for (int i = 0; i < queries_count; i++) {
        int count = 0;
        // Iterate through each input string and compare with the current query using 'strcmp'
